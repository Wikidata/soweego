...
from typing import Iterable, Tuple, TextIO, Union, List, Dict, Callable, Set
...
def cli(catalog, entity, rule, upload, sandbox, dir_io, dates):
    ...
    """
    LOGGER.info(
        "Running baseline '%s' rule over %s %s ...",
        rule, catalog, entity
    )
    ...
    LOGGER.info(
        "Baseline '%s' rule over %s %s completed",
        rule, catalog, entity
    )
...
def _run(catalog, entity, rule, check_dates, upload, sandbox, dir_io):
    ...
    with gzip.open(wd_io_path, 'rt') as wd_io:
        if rule in ('perfect', 'all'):
            ...
            result = _perfect_names_linker(
                wd_io, base_entity, pid, check_dates
            )

            perfect_path = os.path.join(
                dir_io,
                constants.BASELINE_PERFECT.format(catalog, entity)
            )
        ...
        if rule in ('links', 'all'):
            ...
            links_path = os.path.join(
                dir_io,
                constants.BASELINE_LINKS.format(catalog, entity)
            )
            os.makedirs(os.path.dirname(links_path), exist_ok=True)
            _handle_result(result, rule, catalog, links_path, upload,
                           sandbox)
        ...
        if rule in ('names', 'all'):
            ...
            names_path = os.path.join(
                dir_io,
                constants.BASELINE_NAMES.format(catalog, entity)
            )
            os.makedirs(os.path.dirname(names_path), exist_ok=True)
            _handle_result(result, rule, catalog, names_path, upload,
                           sandbox)
                ...
                for wd in bucket:
                    ...
                    for wd_name in wd[keys.NAME]:
                        ...
                        if wd_name.lower() == target.name.lower():
                            if not compare_dates or _birth_death_date_match(wd,
                                                                            target):
...
def _similar_tokens_linker(wd_dataset: TextIO,
                           target_db_entity: Union[BaseEntity, BaseLinkEntity],
                           fields: Tuple[str, str], catalog_pid: str,
                           compare_dates: bool,
                           tokenize: Callable[[str], Set[str]]) -> Iterable[Tuple[str, str, str]]:
        ...
        for wd_name in wd_item[wd_field]:
            ...
            try:
                ...
                for target in data_gathering.tokens_fulltext_search(
                    ...
                    if not compare_dates or _birth_death_date_match(wd_item,
                                                                    target):
                ...
                for target in data_gathering.tokens_fulltext_search(
                    target_db_entity, False, wd_tokens, where_clause=where_clause
                ):
...
def _dates_match(wd_date_and_precision: List, target_date: datetime,
                 target_precision: int) -> bool:
