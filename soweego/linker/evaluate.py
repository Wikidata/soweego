...
@click.option(
    '--nested',
    is_flag=True,
    help='Compute a nested cross-validation with hyperparameters tuning via grid search.',
)
@click.option(
    '--single',
    is_flag=True,
...
def cli(
    ...
    performance_fileout = os.path.join(
        dir_io, constants.LINKER_PERFORMANCE % (target, target_type, classifier)
    )
    predictions_fileout = os.path.join(
        dir_io,
        constants.LINKER_EVALUATION_PREDICTIONS
    ...
    if nested:
        ...
        LOGGER.info(
            'Starting nested %d-fold cross validation with hyperparameters tuning via grid search ...',
            k_folds,
        )
        ...
        for k, model in enumerate(result.pop('best_models'), 1):
            model_fileout = os.path.join(
                dir_io,
                constants.LINKER_NESTED_CV_BEST_MODEL
            ...
            LOGGER.info(
                "Best model for fold %d dumped to '%s'", k, model_fileout
            )
        ...
        LOGGER.info(
            "%s performance dumped to '%s'", metric, performance_fileout
        )
        sys.exit(0)
    ...
    LOGGER.info(
        'Evaluation done. Precision: mean = %s; std = %s; Recall: mean = %s; std = %s; F-score: mean = %s; std = %s',
        p_mean,
        p_std,
        r_mean,
        r_std,
