...
@click.option(
    ...
    default=False,
    help='Dump links gathered from Wikidata. Default: no.',
)
@click.option(
    '--upload/--no-upload',
    default=False,
...
@click.option(
    ...
    type=click.File(),
    default=None,
    help="Load Wikidata links previously dumped via '-w'. Default: no.",
)
@click.option(
    '-d',
    '--deprecated',
    type=click.File('w'),
    default=constants.SHARED_FOLDER + 'entities_deprecated_ids.json',
...
@click.option(
    ...
    '--wikidata',
    type=click.File('w'),
    default=constants.SHARED_FOLDER + 'wikidata_entities.json',
        ...
        json.dump(
            {
                ...
                for qid, data in wikidata_cache.items()
            },
            wikidata,
            indent=2,
            ensure_ascii=False,
...
def check_existence(entity, catalog, wikidata_cache=None):
    ...
    for qid in wikidata:
        ...
        for target_id in identifiers:
            ...
            if not results:
                LOGGER.info(
                    '%s %s identifier %s is invalid', qid, catalog, target_id
                )
...
@click.option(
    ...
    '--deprecated',
    type=click.File('w'),
    default=constants.SHARED_FOLDER + 'links_deprecated_ids.json',
...
@click.option(
    ...
    '--ext-ids',
    type=click.File('w'),
    default=constants.SHARED_FOLDER + 'external_ids_to_be_added.tsv',
...
@click.option(
    ...
    '--urls',
    type=click.File('w'),
    default=constants.SHARED_FOLDER + 'urls_to_be_added.tsv',
...
@click.option(
    ...
    '--wikidata',
    type=click.File('w'),
    default=constants.SHARED_FOLDER + 'wikidata_links.json',
...
def check_links_cli(
        ...
        json.dump(
            {
                ...
                for qid, data in wikidata_links.items()
            },
            wikidata,
            indent=2,
            ensure_ascii=False,
    ...
    json.dump(
        {target_id: list(qids) for target_id, qids in to_deprecate.items()},
        deprecated,
        indent=2,
    )
    ext_ids.writelines(['\t'.join(triple) + '\n' for triple in ext_ids_to_add])
    urls.writelines(['\t'.join(triple) + '\n' for triple in urls_to_add])
    LOGGER.info(
        'Result dumped to %s, %s, %s', deprecated.name, ext_ids.name, urls.name
    )
...
@click.option(
    '--wikidata-dump/--no-wikidata-dump',
    default=False,
    help='Dump metadata gathered from Wikidata. Default: no.',
)
@click.option(
    '--upload/--no-upload',
    default=False,
...
@click.option(
    ...
    type=click.File(),
    default=None,
    help="Load Wikidata metadata previously dumped via '-w'. Default: no.",
)
@click.option(
    '-d',
    '--deprecated',
    type=click.File('w'),
    default=constants.SHARED_FOLDER + 'metadata_deprecated_ids.json',
...
@click.option(
    ...
    '--added',
    type=click.File('w'),
    default=constants.SHARED_FOLDER + 'statements_to_be_added.tsv',
...
@click.option(
    ...
    '--wikidata',
    type=click.File('w'),
    default=constants.SHARED_FOLDER + 'wikidata_metadata.json',
...
def check_metadata_cli(
        ...
        json.dump(
            {
                ...
                for qid, data in wikidata_metadata.items()
            },
            wikidata,
            indent=2,
            ensure_ascii=False,
    ...
    if to_add:
        added.writelines(['\t'.join(triple) + '\n' for triple in to_add])

    LOGGER.info('Result dumped to %s, %s', deprecated.name, added.name)
    ...
    for qid, data in source.items():
        ...
        for target_id in identifiers:
            if target_id in target.keys():
                ...
                if not target_data:
                    LOGGER.warning(
                        'Skipping check: no %s available in target ID %s',
                        criterion,
                        target_id,
                    )
                    continue
                ...
                if extra:
                    LOGGER.debug(
                        '%s has extra %s that will be added to %s: %s',
                        target_id,
                        criterion,
                        qid,
                        extra,
