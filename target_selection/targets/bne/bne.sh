...
# dates: 270,846
for i in {0..271}; do curl -G -H 'Accept:text/csv' --data-urlencode "query=prefix bne: <http://datos.bne.es/def/> select ?person ?dates where { ?person a bne:C1005 ; bne:P5002 ?dates . } OFFSET ""$((i*1000))"" LIMIT 1000" http://datos.bne.es/sparql | tail -n +2 >> dates_people; done
